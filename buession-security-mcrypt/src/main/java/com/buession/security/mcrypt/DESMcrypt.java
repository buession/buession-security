...
public final class DESMcrypt extends AbstractMcrypt {
    ...
    public String encode(final Object object){
        ...
        try{
            ...
            byte[] result = cipher.doFinal(ParseUtils.object2Byte(object, getCharset()));

            if(result != null){
                return ParseUtils.byte2Hex(result);
            }
    ...
    public String decode(final CharSequence cs){
        ...
        try{
            ...
            byte[] result = cipher.doFinal(ParseUtils.hex2Byte(cs.toString()));

            if(result != null){
                return new String(result, getCharset());
            }
        }catch(IllegalBlockSizeException e){
            logger.error(e.getMessage());
        }catch(BadPaddingException e){
    ...
    private final SecretKeyFactory getSecretKeyFactory() throws NoSuchAlgorithmException{
        ...
        return provider == null ? SecretKeyFactory.getInstance(Algo.DES.getName()) : SecretKeyFactory.getInstance
                (Algo.DES.getName(), provider);
    }
